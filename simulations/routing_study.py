...
def main() -> None:
    ...
    for n_uavs in n_values:
        for seed in seeds:
            ...
            viable = links[links.success_probability >= MIN_SUCCESS_PROBABILITY].copy()
            ...
            base = {"seed": seed, "n_uavs": n_uavs, "connected": connected, "graph_edges": graph.number_of_edges()}

            direct = links[((links.tx == source) & (links.rx == target)) | ((links.tx == target) & (links.rx == source))].iloc[0]
            base.update({
                "direct_success_probability": float(direct.success_probability),
                "direct_sinr_db": float(direct.sinr_db),
                "direct_nominal_phy_latency_ms": thesis_profile_50mhz_30khz().slot_duration_ms,
            })
            if connected:
                paths = {
                    "minhop": minimum_hop_path(graph, source, target),
                    "quality": quality_aware_path(graph, source, target),
                    "reliability": reliability_aware_path(graph, source, target),
                }
                for name, path in paths.items():
                    for key, value in _path_metrics(graph, path).items():
                        base[f"{name}_{key}"] = value
            rows.append(base)

    df = pd.DataFrame(rows)
    out = Path("results/routing")
    figs = Path("figures/routing")
    ...
    for n, group in df.groupby("n_uavs"):
        row = {
            "n_uavs": n,
            "seeds": len(group),
            "minimum_link_success_probability": MIN_SUCCESS_PROBABILITY,
            "connectivity_probability": float(group.connected.mean()),
    ...
    ax.plot(summary.n_uavs, summary.connectivity_probability, marker="o")
    ax.set_xlabel("Number of UAVs")
    ax.set_ylabel("Source-target route availability probability")
    ax.set_ylim(0.0, 1.05)
    ax.set_title("Multi-hop route availability vs swarm size")
    ax.grid(True, alpha=0.3)
    fig.tight_layout()
    fig.savefig(figs / "connectivity_vs_density.png", dpi=300)
    fig.savefig(figs / "connectivity_vs_density.pdf")
    plt.close(fig)

    if "reliability_route_success_probability_mean" in summary:
        fig, ax = plt.subplots(figsize=(7.2, 4.8))
        ax.plot(summary.n_uavs, summary.direct_success_probability_mean, marker="o", label="direct")
        ax.plot(summary.n_uavs, summary.reliability_route_success_probability_mean, marker="o", label="reliability-aware route")
        ax.set_xlabel("Number of UAVs")
        ax.set_ylabel("Derived first-TX success probability")
        ax.set_ylim(0.0, 1.05)
        ax.set_title("Direct vs multi-hop reliability abstraction")
        ax.grid(True, alpha=0.3)
        ax.legend()
        fig.tight_layout()
        fig.savefig(figs / "direct_vs_multihop_reliability.png", dpi=300)
        fig.savefig(figs / "direct_vs_multihop_reliability.pdf")
        plt.close(fig)
