...
def main() -> None:
    ...
    # Family 1: fixed area, increasing N.
    fixed_area = []
    for n in n_values:
        for seed in seeds:
            fixed_area.append(_seed_metrics(SwarmConfig(n_uavs=n, seed=seed, area_xy_m=BASE_AREA_SIDE_M)))
    fixed_area_df = pd.DataFrame(fixed_area)
    fixed_area_df["family"] = "fixed_area"

    # Family 2: fixed spatial density, increasing N and area.
    fixed_density = []
    for n in n_values:
        area_side = float(np.sqrt(n / BASE_DENSITY_UAV_PER_M2))
        for seed in seeds:
            fixed_density.append(_seed_metrics(SwarmConfig(n_uavs=n, seed=seed, area_xy_m=area_side)))
    ...
    # Family 3: geometry at one representative density/size.
    geometry_rows = []
    ...
    # Family 4: activity factor vs density.
    activity_rows = []
    ...
    ax.set_title("Fixed area vs fixed spatial density")
